...
if (!function_exists('category_list')) {
	 ...
	 *  category_list($username, $password, $records, $start=0)
	 *
	 *  @subpackage plugin
	 *
	 ...
	 *  @return array
	 */
	function category_list($username, $password, $records, $start=0) {
		...
		$criteria->setSort('`category_type`');
		$criteria->setLimit(intval($records));
		$criteria->setStart(intval($start));
		
		$id=1;
		foreach($categories_handler->getObjects($criteria, true) as $category_id => $category ) {
			$ret[$id] = $category->toArray();
			$id++;
		}	
		return array("category_list" => count($ret), "made" => time(), "data" => $ret);
