...
if (!function_exists('spoofthread')) {

	/*	Xortify Spoof Thread Form generation
	 *  spoofthread($username, $password, $subject, $uri, $ip, $language = 'english')
	 *
	 *  @subpackage plugin
	 *
	 ...
	 *  @return array
	 */	
	function spoofthread($username, $password, $subject, $uri, $ip, $language = 'english') {
		...
		if (!$lang = XoopsCache('xortify_spoofthread_languages')) {
			$lang = array();
			$lang['english']['threadform'] = _SPOOF_CM_THREAD;
			$lang['english']['rules'] = _SPOOF_CM_THREADRULES;
			$lang['english']['rule_text'] = _SPOOF_CM_THREADBANALL;
			$lang['english']['subject'] = _SPOOF_CM_SUBJECT;
			...
			$lang['english']['post'] = _SPOOF_CM_POSTTHREAD;
		}
		...
		XoopsCache::write('xortify_spoofthread_languages', $lang, 60*60*24*7*4*12);
		...
		$form[++$i] = new XoopsFormText($lang[$language]['subject'], 'subject', 50, 255, $subject);
		$form[++$i] = new XoopsFormText($lang[$language]['email'], 'email', 50, 255);
		$form[++$i] = new XoopsFormText($lang[$language]['uname'], 'uname', 15, 35);
		$form[++$i] = new XoopsFormRadio($lang[$language]['msgicon'], 'icon');
		...
		$form[++$i] = new XoopsFormHidden('fct', 'thread');
		$form[++$i] = new XoopsFormHidden('uri', $uri);
		$form[++$i] =  new XoopsFormButton('', 'dopost', $lang[$language]['post'], 'submit');
		
		$cform = new XoopsThemeForm($lang[$language]['threadform'], "threadform", XOOPS_URL.'/spoof/?op=recieve&fct=thread', 'post', true);
		$elements = array();
		foreach($form as $id => $element) {
			$cform->addElement($element, true);
			$elements[$id] = $element->render();
		}
		return array('form'=>$cform->render(), 'elements' => $elements, 'action' => XOOPS_URL.'/spoof/?op=recieve&fct=thread', 'method' => 'post', 'css' => array(XOOPS_URL . '/xoops.css', XOOPS_URL . '/spoof/thread.css'));
	}
}
?>