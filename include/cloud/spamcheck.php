...
if (!function_exists('spamcheck')) {
	 ...
	 *  @return array
	 */
	function spamcheck($username, $password, $puri = '', $content = '', $adult = false, $uname = 'anomynous', $name = 'Guest Anomynous', $email = 'xortify@gmail.com', $ip = '127.0.0.1', $sessid = '', $scores = false) {
		...
		if (!empty($email) && checkemail($email, false) && $email != 'xortify@gmail.com') {
			$email_handler = xoops_getmodulehandler('email', 'noticer');
			$criteria = new CriteriaCompo(new Criteria('email', $email, 'LIKE'));
			$criteria->add(new Criteria('sent', time() - (60*60*24*7*4*2.34), '>'));
			if ($email_handler->getCount($criteria)==0) {
				$reciever = $email_handler->create();
				$reciever->setVar('sender_id', 1);
				$reciever->setVar('email', $email);
				$reciever->setVar('created', time());
				$email_handler->insert($reciever);
			}
		}	
		...
		$header .= "\nSubject: ".$puri;
		$header .= "\nFrom: ".(empty($email)?$uname."@".$_SERVER['REMOTE_ADDR']:$email);
		$header .= "\nTo: $uname"."@".$_SERVER['HTTP_HOST'];
		$header .= "\nDate: ".date('D, d M Y H:i:s +1000');
		$header .= "\n\n";		

		$exe = '/usr/bin/spamc -c --retry-sleep=2 ';
		if ($scores!=false) {
			$exe .= '-r ';
		}
