...
function ban($username, $password, $bans, $comments) {
	...
	foreach ($bans as $key => $ban){	
		if ($ban['network-addy']=='localhost:3307'  ||
			$ban['ip4']=='127.0.0.1' ||
			strpos($ban['ip6'], '127.0.0.1')) {
				$error[$key]['message'] = 'localhost (127.0.0.1) cannot be specified in ban - '.$key;
				$error[$key]['code'] = 'no-localhost';
		}

		if (!isset($error[$key])) {
			if ( !(intval($ban['made'])>time()-(48*60*60)  &&
				 intval($ban['made'])<time()+(48*60*60)) ) {
				 	$error[$key]['message'] = 'ban must be made within '.(48*60*60).' seconds ahead or behind of made server timestamp in ban - '.$key;
				 	$error[$key]['code'] = 'wrong-time';
			}
		}
		if (!isset($error[$key])) {
			foreach($ban as $field => $value) {
				if ($field!='email'&&preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+([\.][a-z0-9-]+)+$/i", $value)) { 
				 	$error[$key]['message'] = 'no email specified allowed for field - '.$field.' - in ban - '.$key;
				 	$error[$key]['code'] = 'email-missing';
				}
					
				if (strpos(' '.$value, '*')) {
					$error[$key]['message'] = 'wildcard * cannot be specified for field - '.$field.' - in ban - '.$key;
					$error[$key]['code'] = 'wildcard-found';
				}
			}
		}
		if (!isset($error[$key])) {
	...
	foreach ($create as $key => $ban){
		...
		$criteria->add(new Criteria('email', $banning->getVar('email')), "OR");
		...
		if ($itemid = $banmemberHandler->insert($banning, true)) {
			$error[$key]['message'] = 'Ban Successfully Created ~ ' . $key;
			$error[$key]['code'] = 'success';
			$error[$key]['ban_id'] = $itemid;
	...
	return array("bans" => count($create), "errors" => $error, "made" => time());
}
