...
switch($mode) {
	case 'soap':
		...
		if (is_object($pluginObj)) {
			include_once($GLOBALS['xoops']->path('modules/xrest/plugins/'.$pluginObj->getVar('plugin_file')));
		}
	...
	default:
		...
		if (isset($plugin)) {
			...
			if (is_object($pluginObj)) {
				...
				if (/*(!$result = XoopsCache::read('xrest_results_' . sha1($plugin . serialize($values)))) &&*/ $pluginObj->getVar('active')==true) {
					$result = array();
					$opfunc = $plugin;
					...
					if (!empty($opfunc)) {
						...
						// Cache's Result Set
						XoopsCache::write('xrest_results_' . sha1($plugin . serialize($values)), $result, $GLOBALS['xrestModuleConfig']['cache_seconds']);
					}
				}	
			}
		}
